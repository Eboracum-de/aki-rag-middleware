...
There is no automated cross-service consistent-restore command in 0.8.5-rc4.2. A conservative manual order is:
...
The current credential format is versioned and authenticated, but 0.8.5-rc4.2 does not provide a documented zero-downtime master-key rotation workflow for all encrypted rows.
